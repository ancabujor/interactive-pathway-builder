      ...
      <div className="flex justify-center items-center space-x-4 sm:space-x-8">
        ...
        <div className="flex flex-col items-center">
          <div className={`step-indicator flex items-center justify-center 
            ...
            {currentStep > 3 ? <CheckIcon className="h-4 w-4" /> : '3'}
          </div>
          <span className={`text-xs mt-1 text-center ${currentStep === 3 ? 'font-medium text-blue-600' : ''}`}>Test AI Receptionist</span>
        </div>
        <div className={`w-12 h-px ${currentStep >= 4 ? 'bg-blue-500' : 'bg-muted-foreground/30'}`} />
        <div className="flex flex-col items-center">
          <div className={`step-indicator flex items-center justify-center 
            ${currentStep === 4 ? 'bg-blue-500 text-white font-bold ring-2 ring-blue-300 ring-offset-2' : ''}`}>
            4
          </div>
          <span className={`text-xs mt-1 text-center ${currentStep === 4 ? 'font-medium text-blue-600' : ''}`}>Selection</span>
        </div>
      </div>
    </header>
  );
